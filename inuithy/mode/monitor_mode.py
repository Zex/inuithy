...
class MonitorController:
    ...
    @property
    def initialized(): return MonitorController.__initialized
    @initialized.setter
    def initialized(val):
...
#        return None
    ...
    def start(self):
        if not MonitorController.initialized:
            self.lg.error(string_write("MonitorController not initialized"))
            return
        try:
...
#            if self.__traffic_timer != None: self.__traffic_timer.start()
            ...
            self.lg.info(string_write("MonitorController received keyboard interrupt"))
        except Exception as ex:
            self.lg.error(string_write("Exception on MonitorController: {}", ex))
        finally:
            self.teardown()
            self.lg.info(string_write("MonitorController terminated"))
...
def start_controller(tcfg, trcfg):
    controller = MonitorController(tcfg, trcfg, lg)
    controller.start()
