...
#        self.cond = threading.Condition()
    ...
    def set_all(self):
        _l.info("Notify all waiting processes")
        try:
            [e.set() for e in [
            self._is_agents_up,
...
#           self._is_agents_unregistered,
            ...
            _l.error("Failed to set all waiting")

    def clear_all(self):
        _l.info("Clear all waiting processes")
        try:
            [e.clear() for e in [
            self._is_agents_up,
            self._is_nwlayout_done,
    ...
    def create_nwlayout(self, nwinfo):#nwid, nodes):
        ...
        _l.info("Create network layout checker")
        if nwinfo is None:
            return
...
#        _l.info("Is network layout done")
        try:
...
#            _l.debug(self.traffic_stat)
            ...
            _l.error(_s("Failed to check whether traffic registered: {}", ex))
            return False

    def is_phase_finished(self):
        """Whether traffics are finished"""
#        _l.info("Is traffic all finished")
        try:
            if len(self.available_agents) == 0:
                raise RuntimeError("No agent available")
            if len(self.traffic_stat) == 0:
            ...
            _l.error(_s("Failed to check whether traffic finished: {}", ex))
            return False

    def is_agents_up(self):
        """Whether expected agents all started"""
#        _l.info("Check for agents availability")
        try:
...
#                    return False
            ...
            _l.error(_s("Failed to check agent availability", ex))
            return False
...
def transition(sm, event):
    ...
    try:
        ...
        _l.error(_s("Failed to transite status {}: {}", event, ex))
    ...
    for subnet in nwcfg.config.get(nwlayoutname).values():
        ...
        for node in subnet.get(T_NODES):
            ...
            if target_host is None:
                raise RuntimeError(_s("Node [{}] not found on any agent", node))
...
class TrafficState:
    ...
    def __init__(self, controller, trdelay=1):
        """
    ...
    def record_genid(self, genid):
        ...
        try:
            ...
            _l.error(_s("Record genid failed: {}", ex))
    ...
    def check(self, cond):
        """Check for condition"""
        _l.info(_s("Current state: {}, check for: {}", str(self.current_state), cond))
    ...
    def do_waitfor_agent_all_up(self):
        ...
        if not self.traf_running:
            return
        _l.info(_s("Wait for agents get ready", str(self.current_state)))
        _c("Wait for agents get ready")
        try:
            self.chk._is_agents_up.wait()
        except KeyboardInterrupt:
            _l.info("Terminating ...")
        except Exception as rex:
            _l.error(_s("Wait for agent up: {}", ex))
    ...
    def do_waitfor_nwlayout_done(self):
        ...
        if not self.traf_running:
            return
        _l.info(_s("Wait for network layout done: {}", str(self.current_state)))
        _c("Wait for network layout done")
        try:
            self.chk._is_nwlayout_done.wait()
        except KeyboardInterrupt:
            _l.info("Terminating ...")
        except Exception as ex:
            _l.error(_s("Wait for network layout done: {}", ex))
    ...
    def do_waitfor_traffic_set(self):
        ...
        if not self.traf_running:
            return
        _l.info(_s("Wait for traffic registered: {}", str(self.current_state)))
        _c("Wait for traffic registered")
        try:
            self.chk._is_traffic_registered.wait()
        except KeyboardInterrupt:
            _l.info("Terminating ...")
        except Exception as rex:
            _l.error(_s("Wait for get traffic registered: {}", ex))

    @before('create')
    def do_create(self):
        if not self.traf_running:
            return
        _l.info(_s("Create traffic from configure: {}", str(self.current_state)))
        self.phases = create_phases(rt.trcfg, rt.nwcfg)
        _l.info(_s("Total phase: [{}]", len(self.phases)))
    ...
    def do_start(self):
        ...
        if not self.traf_running:
            return
        _l.info(_s("Start traffic sm: {}", str(self.current_state)))
        try:
            self.create()
            _l.info("Starting agents ...")
            ...
            try:
                ...
                _l.info("All traffic generation done")
#            [self.start_phase() for tg in self.next_phase]
        except KeyboardInterrupt:
            _l.info("Terminating ...")
        except Exception as ex:
            _l.error(_s("Traffic runtime error: {}", ex))
        self.finish()
    ...
    def record_phase(self):
        ...
        _l(_s("Current phase: {}", self.current_phase))
    ...
    def start_phase(self):
        ...
        try:
            ...
            _l.error(_s("Traffic state transition failed: {}", str(ex)))
    ...
    def do_deploy(self):
        ...
        if not self.traf_running:
            return
        _l.info(_s("Deploy network layout: {}", str(self.current_state)))
        phase = self.current_phase
        #TODO deploy with coordinator
        ...
        _l.info(_s("Current network layout: {}", self.ctrl.current_nwlayout))
    ...
    def do_register(self):
        ...
        if not self.traf_running:
            return
        _l.info(_s("Register traffic task: {}", str(self.current_state)))
        phase = self.current_phase
        _l.info(_s("Register traffic: [{}]", str(phase)))
    ...
    def do_fire(self):
        ...
        if not self.traf_running:
            return
        _l.info(_s("Fire traffic: {}", str(self.current_state)))
    ...
    def do_phase_finish(self):
        ...
        if not self.traf_running:
            return
        _l.info(_s("Traffic finished: {}", str(self.current_state)))
        try:
            ...
            _l.error(_s("Traffic runtime error: {}", rex))

    @after('genreport')
    def do_genreport(self):
        """Analyze collected data and generate report"""
        _l.info(_s("Try analysing: {}", str(self.current_state)))
        if not self.traf_running:
            return
        if rt.tcfg.storagetype in [\
            ...
            _l.info(_s("Unsupported storage type: {}", str(rt.tcfg.storagetype)))

    @after('finish')
    def do_finish(self):
        """All traffic finished"""
        _l.info(_s("All finished: {}", str(self.current_state)))
#        if not self.traf_running:
#            return
...
#            self.chk.set_all()
            ...
            _l.error(_s("Exception on stopping agents: {}", ex))

        try:
            stop_sniffer()
        except Exception as ex:
            _l.error(_s("Exception on stopping sniffer: {}", ex))

        _l.info("Stopping controller ...")
        self.ctrl.teardown()
...
#        self.chk.done.clear()
        ...
        try:
            if len(self.chk.available_agents) > 0:
                _l.info("Force stopping agents ...")
                force_stop_agents(self.chk.expected_agents)
        except Exception as ex:
            _l.error(_s("Exception on force stopping agents: {}", ex))

        _l.info("Agents stopped")
